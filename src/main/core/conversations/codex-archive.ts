...
const CODEX_ARCHIVE_TIMEOUT_MS = 10_000;
...
export async function ensureCodexThreadArchived({
  ...
  try {
    ...
    await ctx.exec(command.command, command.args, {
      timeout: CODEX_ARCHIVE_TIMEOUT_MS,
      maxBuffer: CODEX_ARCHIVE_MAX_BUFFER,
    });
  } catch (error) {
    log.warn('ensureCodexThreadArchived: failed to archive Codex thread', {
      threadId,
      error: String(error),
    });
  }
